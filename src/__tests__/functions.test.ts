...
describe("src.functions", () => {
  ...
  it("should resize width only, keeping aspect ratio", () => {
    const { resizedCanvas } = resize({canvas: originalCanvas, options: { width: newWidth}});
    expect(resizedCanvas?.width).toEqual(newWidth);
    expect(resizedCanvas?.height).toEqual(newWidth / (originalWidth / originalHeight));
  });

  it("should resize height only, keeping aspect ratio", () => {
    const { resizedCanvas } = resize({canvas: originalCanvas, options: { height: newHeight}});
    expect(resizedCanvas?.height).toEqual(newHeight);
    expect(resizedCanvas?.width).toEqual(newHeight * (originalWidth / originalHeight));
  });

  it("should resize both dimensions, keeping aspect ratio when dimensions match original ratio", () => {
    const { resizedCanvas } = resize({canvas: originalCanvas, options: { width: newWidth, height: newHeight}});
    expect(resizedCanvas?.width).toEqual(newWidth);
    expect(resizedCanvas?.height).toEqual(newHeight);
  });
  ...
  it("should handle null dimensions correctly", () => {
    const { resizedCanvas: resultWidthNull } = resize({canvas: originalCanvas, options: { width: null, height: newHeight }});
    expect(resultWidthNull?.width).toEqual(300 * (originalWidth / originalHeight));
    expect(resultWidthNull?.height).toEqual(newHeight);

    const {resizedCanvas: resultHeightNull} = resize({canvas: originalCanvas, options: { width: newWidth, height: null }});
    expect(resultHeightNull?.width).toEqual(newWidth);
    expect(resultHeightNull?.height).toEqual(400 / (originalWidth / originalHeight));
  });

  it("should return null when canvas context cannot be created", () => {
    // Мокаем getContext чтобы вернуть null
    const mockCanvas = { ...originalCanvas, getContext: () => null } as HTMLCanvasElement;
    const { resizedCanvas } = resize({canvas:mockCanvas, options: { width: 100 }});
